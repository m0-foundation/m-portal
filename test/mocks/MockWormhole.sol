...
/// @dev This mock is forked from the Wormhole Solidity SDK
///      It has added functionality to track emitter sequences
///      within tests.
contract MockWormhole is IWormhole {
    ...
    // This function is made public so you can manually increment sequences in tests
    // without publishing a message
    function useSequence(address emitter) public returns (uint64 sequence) {
        sequence = _sequences[emitter]++;
    }
    ...
    function verifyVM(VM memory vm) external view override returns (bool valid, string memory reason) {}
    ...
    function quorum(uint numGuardians) external pure override returns (uint numSignaturesRequiredForQuorum) {}
